...
const regex = __importStar(require("../lib/regex.js"));
/** @type LanguageFn */
function default_1(hljs) {
    ...
    const makeOperatorMode = function ({ includeEqual }) {
        ...
        const SYMBOLIC_OPERATOR_RE = regex.either(regex.concat(OPERATOR_FIRST_CHAR_OF_MULTIPLE_RE, OPERATOR_CHAR_OR_DOT_RE, '*'), // Matches at least 2 chars operators
        regex.concat(OPERATOR_CHAR_RE, '+'));
        return {
            scope: 'operator',
            match: regex.either(
            // symbolic operators:
    ...
    const CHAR_LITERAL = {
        scope: 'string',
        match: regex.concat(/'/, regex.either(/[^\\']/, // either a single non escaped char...
